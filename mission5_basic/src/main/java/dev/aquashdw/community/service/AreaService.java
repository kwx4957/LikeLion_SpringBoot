...
public class AreaService {
    ...
    public AreaDto readAroundArea(double latitude, double longitude){
        double minD = 1.7976931348623157E308;
        AreaDto temArea = null;
        List<AreaDto> areaDtoList = new ArrayList<>();
        areaRepository.findAll().forEach(areaEntity -> areaDtoList.add(new AreaDto(areaEntity)));

        for (AreaDto dto : areaDtoList) {
            if (measureDistance(latitude,longitude,dto) < minD){
                temArea = dto;
                minD = measureDistance(latitude,longitude,dto);
            }
        }
    ...
    private double measureDistance(double latitude, double longitude, AreaDto areaEntity){
        double d;
        double xD, yD;
        xD = Math.pow((areaEntity.getLatitude()-latitude),2);
        yD = Math.pow((areaEntity.getLongitude()-longitude),2);
        d = Math.sqrt(yD+xD);
        return d;
    }
}
